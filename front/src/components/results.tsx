...
export function Results() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => fetchBusinessByFilters({data_de_partida: dateFrom, local_partida: from, pagina: 1, qtd_passageiros: numberPassengers})
  })

  return (
    <div className="w-full grid grid-cols-4 gap-3 mt-8">
      {data?.map((empresa) => (
        <SearchResults address={empresa.endereco} fantasyName={empresa.nome_fantasia} idBusiness={empresa.id_usuario} phone={empresa.telefone} rate={empresa.soma_avaliacoes} key={empresa.id_usuario} photo={empresa.foto}/>
      ))}
